...
private:
    ...
    const float WALL_THICKNESS_SCALE = 0.4f; 
    const float WALL_DEFAULT_DISTANCE = 6.0f;
...
inline void StoneWall::Draw(const glm::mat4 &projection, const glm::mat4 &view, const glm::vec3 &viewPos)
{
    ...
    // 🔴 ตำแหน่ง (Translation)
    glm::vec3 segmentWorldPos = m_centerPosition; 
    ...
    modelMatrix = glm::scale(modelMatrix, glm::vec3(lengthScale, m_maxHeight, thicknessScale));
